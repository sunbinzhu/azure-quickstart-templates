...
function InstallHPCBrokerNode
{
    ...
    TraceInfo "Installing HPC Pack broker node from \\$clustername\REMINST"
    $pSetup = Start-Process -FilePath "\\$clustername\REMINST\setup.exe" -ArgumentList "-unattend -brokernode:$clustername" -Wait -PassThru
    if(($pSetup.ExitCode -ne 0) -and ($pSetup.ExitCode -ne 3010))
    {
        throw "Failed to install broker node: $($pSetup.ExitCode)"
    }
    TraceInfo "Succeeded to install HPC Pack broker node."
...
{   
    ...
    {
        TraceInfo "Start to install broker node"
        InstallHPCBrokerNode $ClusterName
        TraceInfo "Finish to install broker node"
    }
